...
**Status (Session 6):** Phase 6 res surface MERGED (differential oracle caught a real Phase 3 bug: Content-Disposition wrong on 14/15 filenames, including a filesystem-path leak into a header — replaced with an RFC 6266/5987 module pinned against `content-disposition@0.5.4`). **W2-V PASSED with a narrowed claim:** the win is table-size scaling, not general routing — at 6 routes Fastify leads 0.97×; at 200 zonix leads 1.40×; 6→200 costs zonix 1.1%, Express 10.1%, Fastify 31.3% (a cliff between 50–100 routes, `process.nextTick` 0.96% → 21.94%; four alternative explanations tested and rejected; Fastify internals not root-caused). Rule 8 (oracle tests) added. **Still pending on Swapnil: the AV exclusion — W1 remains frozen.** **T-0 Turbo spike EXECUTED in-container: kill bar PASSED (12.63× corked / 1.78× uncorked vs raw `node:http`, 8/8 pairs above bar) — Turbo lives pending reference-rig confirmation.** **Next: (1) re-run the delivered spike paired on the reference rig — the official T-0 number; (2) Phase 6 exit test; (3) upstream repro draft + M3 footprint bench; (4) Turbo design doc per M4; then Phase 7 (M1 static stack).**
