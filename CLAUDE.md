...
**Status (Session 9): TURBO KILLED at T-1.** Judged cell: **1.362× vs raw `node:http` against the 1.40× bar — FAILED** (pairs 1.29–1.39, the bar never touched); 1.392× vs Fastify (its 1.30× bar cleared, but D7's AND condition decides); 1.408× vs zonix (informational). Death is final: configuration pre-committed, median of five paired rounds, no re-rolls; falsification record at `t1/`, TURBO.md banner reads KILLED, commit 1dc57a2. Erosion finding: honest parsing + HOL ordering + per-request response building ate ~20% of the spike's margin (1.71× → 1.36×). **Crown per D7's own pre-written sentence: M1–M3 + parity.** **Next: one session — regime preflight verdict (did the AV exclusion land?), M3 footprint bench, upstream drafts (Fastify cliff repro + Express docs PR) for review; then Phase 7 (negotiator first, oracle tests from day one, W1 adjudication if the regime is clean).**
