...
**Phase 5.5 CLOSED. BI-1 CLOSED** — verdict: the anomaly was the *machine*, not the competitors (system-wide `open()` throttling from a filesystem filter driver / AV real-time scanning; see Session 4). All absolute file-rps claims withdrawn, including our own 23.0k; F1's paired **+98.4%** survives with mechanism. **Phase 6 restructure commit merged** (165 tests green both Node versions, export surface byte-identical). **Manual task for Swapnil: add an AV real-time-scanning exclusion for the repo + bench fixture directories** — until then exit (c) is unadjudicable. **Next: regime preflight in the harness, fastify-schema confirmation, then Phase 6 compat surface (req side first), per the Beat-Fastify program (W-criteria, Session 4).**
