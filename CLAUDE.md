...
**Status (Session 12): D8 — the bench moves into a container.** Diagnosis came back decisive on one front and forked on the other: the harness context is **native win32** (WSL/bridge hypothesis dead), and **no exclusion is in effect there** — repo and `%TEMP%` both degraded (~4–4.7k), identical with the sandbox disabled; Tamper Protection is ON and the machine rebooted at 13:36. **Fork RESOLVED same day (interactive re-run, matching fingerprint): the same Git Bash context that read ~48k @ 12.5× now reads 4,339 @ 142.4× — "two machines" were two *times*.** The exclusion was live in the Session-10 window and reverted after the reboot; with the exclusion list admin-only, the likely cause is managed Defender policy re-syncing, meaning manual exclusions will not persist on this host. **Host exclusion: classified UNSTABLE — dev-comfort pursuit only.** Docker verified (28.4.0, hello-world green) — **the D8 container session is GO.** **Detector port DONE** (`583d72c`): one shared constant, fingerprints, pre/post REGIME-FLIP, exercised live. **Fastify finding changed shape:** the minimal repro is just `Fastify({logger:false})` + N param routes — every prior "trigger ingredient" was the pre-reboot sick machine — but the effect is **machine-state-dependent, sign included** (cliff in fast bands, +14–27% inversion in slow bands, order-reversal controlled), while **zonix reads 0.970/1.010 — flat through every state and order: W2's zonix half got stronger, Fastify's half is not filable on single-machine sign-flipping evidence.** New standing rules recorded (rule 9): table-size claims carry a zonix flat-control in the same run; sign-sensitive claims need a second environment. **D8 (binding): a pinned Docker container becomes the official environment for file scenarios and cross-framework claims** — repo COPIED in, never bind-mounted from `C:\` — giving a Defender-free, reproducible, publishable bench context on demand; the host Defender mystery becomes a dev-comfort item, no longer a program blocker. **Next: container session — Dockerfile + runner, regime probe inside (expect ext4-fast), exit (c) file adjudication at last, Fastify repro + flat-control across two windows; Phase 7 (negotiator first) the session after.**
