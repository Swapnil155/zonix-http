...
**Status (Session 11): THE MACHINE IS TWO MACHINES — W1 re-frozen pending context diagnosis.** Two contradictory, probably-both-true measurements of the same repo: Swapnil's interactive Git Bash reads **~48k opens/sec @ 12.5×** (clean, differential-verified against `%TEMP%`); the Claude Code session context reads **3,897/4,617/4,506 @ 123–163×, system-wide** (fixture, repo, tmpdir all ~4k — the same band as every session since BI-1). The exclusion is real *in one execution context* and the benchmarks run *in the other*. Hidden variable, now controlled: **rule 7 gains a mandatory context fingerprint** (platform, `os.release()`, cwd, `os.tmpdir()`, `process.execPath`) recorded with every regime reading. Leading hypothesis: the session context is WSL (bridge/translation overhead produces exactly this signature regardless of Defender); alternative: exclusion reverted or per-context — the fingerprint + an in-session repo-vs-`%TEMP%` differential decides. **The detector port (shared constant, REGIME-FLIP, new thresholds) was never executed — the queued command wasn't run — and now includes the fingerprint.** Also this session: **M3 DELIVERED** (65× install, 407× files, 56-vs-1 packages, 4.3–4.8× cold import, RSS honestly only 1.20× vs Fastify; first-import-after-install on an AV machine: 21.6ms vs 1,240/1,487ms); **Express docs PR APPROVED for filing**; **Fastify issue correctly held** — recorded harness reproduces (−24%, third consistent session) but a from-scratch server does not, trigger pinned to something in `bench/servers/fastify.js`, isolation stopped on a new 40% intra-config wobble with CPU preflight green (new blind spot, likely the same context mystery). **W2 publication now additionally gated on the standalone repro.** **Next: diagnosis session — fingerprint + in-session differential first, then the detector port, then Fastify isolation on a quiet machine; Phase 7 opens only once the regime question has one answer per context.**
