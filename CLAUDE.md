...
**Status (Session 8):** **Phase 6 CLOSED** — exit test (two halves, one corpus: readable assertions + the same handlers wire-diffed against real Express) found and fixed five real defects; 428 tests green; gate PASS +0.35%. **T-0 official on the reference rig: p=1 → 1.71× raw `node:http` (container predicted 1.78× — transferred within 4%); p=16 → 10.78×** (colocation-inflated as forecast; judged number for production is ~1.65× per TURBO.md). New finding: **C=1 ratio is 1.16× — Turbo is a throughput engine, not a latency engine (binding README language).** TURBO.md design doc delivered. **D7: Turbo's bar RAISED — T-1 must clear ≥1.40× vs raw and ≥1.30× vs Fastify, shim-inclusive, or Turbo dies.** Local claims are Node 22-only until Phase 9 CI owns the matrix. **Next: T-1 (one session, alone, D7 bars); then M3 + upstream filings (Fastify cliff repro, Express docs PR); then Phase 7 — regime preflight decides whether the AV exclusion actually landed.**
