...
**Status (Session 13): EXIT (c) MET — and the Fastify mystery has a face.** Container is live and clean by construction (entry probe 582–620k opens/sec @ 5.6× vs host ~4k @ 125× — a ~140× courtroom gap on the same hardware). **file-1kb, rotating order, 5 rounds, regime clean pre+post: zonix 12,370 / express 7,117 / fastify 8,647 → 1.74× Express, 1.43× Fastify — exit (c) cleared decisively** (ratios are the claim, container absolutes never are). **The Fastify effect is BIMODAL per-process throughput, not a cliff:** a fast mode (~165k) that small tables sometimes reach (2/12 container; the default in host fast bands) and 200-route tables were never observed to reach (0/12) — no per-request cost grows with the table. That one fact explains every cliff/flat/inversion of four sessions, including the nextTick signature. **zonix: 141–150k, 0.97–1.01, no modes, every window and order — determinism itself is now a measured differentiator.** Ratified: W2 retires the word "cliff"; ISSUE.md reframed bimodal; one ROUNDS=20 mode-frequency sampling before filing. Full consolidated scorecard in the Session 13 addendum. **Next: Phase 7 opens — negotiator pinned as rule-8 oracle before anything wires into `req.accepts`/`res.format`; the ROUNDS=20 sampling rides along.**
